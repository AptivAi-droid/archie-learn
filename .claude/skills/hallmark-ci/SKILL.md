---
name: hallmark-ci
description: "Anti-AI-slop design skill that emits CodeIgniter 4 views for cPanel PHP 8.2+ projects. Use when building, redesigning, auditing, or extracting the design of a page/view/component in a CodeIgniter app, when the user invokes Hallmark by name, or uses audit/redesign/study. Output is CI4 view templates (layouts, sections, partials) plus static CSS/JS assets under public/, not standalone HTML or React."
metadata:
  version: 1.1.0-ci
  upstream: https://github.com/Nutlope/hallmark
  edition: Aptiv Consulting — CodeIgniter 4 / cPanel PHP 8.2+
---

# Hallmark (CodeIgniter 4 edition)

A design skill for AI coding assistants. Makes the UIs they generate look made, not generated — and ships them as **CodeIgniter 4 views that run on cPanel shared hosting with PHP 8.2+**.

This is the Aptiv Consulting edition of [Hallmark](https://github.com/Nutlope/hallmark). The design brain is unchanged — same 21 themes, 21 macrostructures, 58-gate slop test, typography/colour/component systems. Only the **output layer** is re-targeted: every artifact is emitted as a CodeIgniter 4 view (or view partial) with its styling in static asset files, wired through the framework's helpers. Read the binding output contract in [`references/codeigniter.md`](references/codeigniter.md) — it governs every verb below.
...
**Powered by Together AI.** CodeIgniter edition maintained by Aptiv Consulting.

---

## Output target — read this before any verb

Every emit in this skill lands in a **CodeIgniter 4** project served from **cPanel shared hosting on PHP 8.2+**. This is not negotiable and it is not detected — it is the house target. Wherever the design brain or a reference file says "emit HTML", "emit a `.tsx`/`.jsx`", "write `index.html`", or "self-contained page", read it as: **emit the equivalent CodeIgniter 4 view.** The full contract — folder layout, view syntax, escaping, asset URLs, deployment — lives in [`references/codeigniter.md`](references/codeigniter.md). Load that file at the start of every default build, redesign, and component run. The rules in brief:

- **Markup → CI4 views.** A page becomes a view in `app/Views/` that `$this->extend()`s a layout and fills `$this->section()`s. A reusable block becomes a partial in `app/Views/partials/` (or `app/Views/components/`) pulled in with `$this->include()`. Never emit a standalone `.html` file as the deliverable; the demo/preview wrapper is the only HTML that ever leaves the skill, and it is explicitly throwaway.
- **Styling → static assets, not a build step by default.** Hand-written CSS ships to `public/assets/css/` and is referenced with `<?= base_url('assets/css/…') ?>`. `tokens.css` is still the source of truth; it lives at `public/assets/css/tokens.css` and the page CSS `@import`s it (or the layout links both). Any JS is plain vanilla in `public/assets/js/`. No npm/Node is required to run the output on cPanel. An optional Vite pipeline is documented for teams that have Node locally and commit compiled assets — see [`references/codeigniter.md` § Optional build pipeline] — but the default path assumes none.
- **Dynamic values → framework helpers, always escaped.** All URLs go through `base_url()` / `site_url()`; all echoed data goes through `esc()`; forms use CI4's `csrf_field()` and `form helper` where relevant. Static design copy the user supplied can be literal; anything that could be data must be escaped.
- **The design brain is unchanged.** Genre detection, macrostructure pick, theme rotation, the 58 slop-test gates, mobile non-negotiables, typography purity, honest copy — all apply exactly as written. CI4 changes *where the bytes land and how they're wired*, never the taste rules.

If a project is on **CodeIgniter 3** instead, [`references/codeigniter.md` § CodeIgniter 3 fallback] gives the syntax deltas (`$this->load->view()`, `html_escape()`, `application/views`), but CI4 is the default assumption unless the pre-flight scan proves otherwise.
...
1. **The component artifact** — a CodeIgniter 4 view partial plus its styling:
   - Markup: `app/Views/components/button.php` — a partial pulled into pages with `<?= $this->include('components/button', ['label' => $label]) ?>`. Accept data through the `$data` passed to `include()`; escape every dynamic value with `esc()`; build any URL with `base_url()`. Keep parameters explicit at the top of the partial in a short doc comment.
   - Styling: append the component's rules to `public/assets/css/components.css` (or a dedicated `public/assets/css/components/button.css` if the project keeps them split), consuming Hallmark tokens by name (`var(--color-accent)`) — never inline OKLCH values.
   - If `public/assets/css/tokens.css` doesn't exist yet, create it (see [`references/codeigniter.md`](references/codeigniter.md)).

2. **An 8-state demo wrapper** — a throwaway CI4 route + view, or a single standalone `<component>.preview.html`, that renders the component in **all 8 states** stacked vertically, each labelled. Prefer a temporary view at `app/Views/_preview/<component>.php` reachable through a dev-only route (state the route to add, e.g. `$routes->get('_preview/button', 'Preview::button')`), so it renders through the real layout and asset pipeline; a standalone `.preview.html` is the fallback when adding a route is unwelcome. Either way it is **not production code** — the user opens it once, confirms the states, then deletes the view and route. This preview wrapper is the only HTML the skill ever emits as a file. Format:
...
If the project already has code — a `composer.json`, an `app/Views/` tree, a `spark` binary, a `public/index.php`, any CSS in `public/assets/` — Hallmark should **read it before asking the user anything**. Stomping on an established palette, font stack, or view/layout structure is the difference between a skill the user keeps and a skill the user uninstalls. In a CodeIgniter project, read the existing layout(s), any `app/Views/partials/`, and the current `public/assets/css/` so new views extend what's there instead of reinventing it.
...
1. **Font stack** — any `<link rel="stylesheet" href="...fonts.googleapis.com/...">` or `@font-face` in the layout views under `app/Views/` (esp. a `layouts/` folder); `@import url("fonts.googleapis.com/...")` or `@font-face` in `public/assets/css/`; self-hosted font files under `public/assets/fonts/`. (Legacy JS signals — `next/font`, `@fontsource/*`, `tailwind.config` `fontFamily` — only matter if the project turns out not to be CodeIgniter.)
...
5. **Framework** — **CodeIgniter 4** is the house default: confirm it from `composer.json` (`codeigniter4/framework`), the `app/` + `public/` + `spark` layout, and the CI version in `system/CodeIgniter.php` or `composer.lock`. **CodeIgniter 3** shows as `application/` + `system/` with `application/config/config.php` and no `composer.json` framework entry — if you see this, switch to the CI3 syntax deltas in [`references/codeigniter.md`](references/codeigniter.md). Only note a non-CI stack (Next.js, Astro, Vue, etc.) if the evidence is unambiguous, and then ask the user how they want it emitted — this edition is built to ship CodeIgniter views.

**Output format** — emit this block once, before Step 1, with file:line citations so the user can verify what you found:

```
Pre-flight findings:
· Framework: CodeIgniter 4.5 (composer.json, spark present)
· Layout: app/Views/layouts/main.php extends pattern in use
· Font stack: Fraunces + Geist (Google Fonts <link>, app/Views/layouts/main.php L9)
· Palette: OKLCH custom properties (public/assets/css/tokens.css :root)
· Motion: none detected (no JS libs; motion-cut project)
· Spacing: --space-* scale present (public/assets/css/tokens.css)

Hallmark will preserve: layout structure, font stack, palette, spacing scale.
Hallmark will introduce: macrostructure, microinteraction discipline,
slop-test gates, hero enrichment recipe — emitted as CI4 views + public/assets.
...
- [`codeigniter.md`](references/codeigniter.md) — **the binding output contract for this edition.** CI4 view/layout/section/partial syntax, `esc()` / `base_url()` usage, the `public/assets/` structure, where `tokens.css` lives, the optional Vite build, the CI3 fallback, and cPanel deployment notes. Every emit lands here — load it first on every default build, redesign, and component run.
...
- **Emit the markup as CodeIgniter 4 views, not standalone HTML.** The page becomes a view in `app/Views/` that `$this->extend()`s a layout and fills its `$this->section('content')`; shared chrome (nav, footer, `<head>`) lives in the layout and in `app/Views/partials/`. Escape every dynamic value with `esc()`, build every URL with `base_url()` / `site_url()`, and state the route the user should register for the page. If no layout exists yet, create `app/Views/layouts/main.php` and link the assets there. The full pattern — with worked view/layout/partial examples — is in [`references/codeigniter.md`](references/codeigniter.md); load it now if you haven't. All the taste rules below apply to the markup inside these views unchanged.
...
- **Never clobber an existing stylesheet.** When the project already ships CSS in `public/assets/css/` (a `site.css`, `app.css`, or per-page files), it is **append-only**: keep the existing rules, add Hallmark's page rules below them, keep any new `@import "tokens.css"` at the very top of the file above all other rules (CSS drops a misplaced `@import` silently), and reuse the project's own token names where they exist. Overwrite a file only if the user explicitly asks. Likewise never rewrite an existing layout view wholesale — extend it or add a new layout. See [`contract.md`](references/contract.md) and [`codeigniter.md`](references/codeigniter.md).
- **Always emit `tokens.css`.** After writing the page CSS, also write `public/assets/css/tokens.css` containing every `--color-*`, `--font-*`, `--space-*`, `--text-*`, `--ease-*`, `--dur-*`, `--rule-*`, and `--radius-*` token used in the build. The page CSS `@import`s it at the top (or the layout links both files in order, tokens first) — the page CSS must reference tokens by name, never inline raw values. Reference it in views with `<?= base_url('assets/css/tokens.css') ?>`. Even single-page builds get a `tokens.css`. This is what makes the design system portable to the next project. Load [`export-formats.md`](references/export-formats.md) at this point only when the project warrants additional formats — see below.
