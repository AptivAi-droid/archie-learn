...
> **CodeIgniter edition note.** In this edition, `tokens.css` — the source of truth — is written to **`public/assets/css/tokens.css`**, not the project root, and the layout links it (tokens first) via `base_url()`. See [`codeigniter.md`](codeigniter.md). The token taxonomy and OKLCH values below are unchanged; only the file location differs. Formats 2–4 (Tailwind `@theme`, DTCG `tokens.json`, shadcn/ui variables) are **optional** here — emit them only when the specific project actually uses that tooling, or into a `design.md` on the user's request. A plain CI4 + cPanel project needs only `tokens.css`.
...
1. **Always** write `tokens.css` to `public/assets/css/tokens.css` (CI edition) and link it from the layout before every other stylesheet. Format 1.
